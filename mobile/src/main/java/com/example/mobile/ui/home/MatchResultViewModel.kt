...
class MatchResultViewModel(private val repository: MatchRepository): ViewModel() {
    ...
    fun loadHeatMap(){
        viewModelScope.launch {
            _heatMapUrl.value = repository.loadPersonalHeatMap()
        }
    }

    fun loadReplay(){
        viewModelScope.launch {
            _replayUrl.value = repository.loadPersonalReplay()
        }
    }
}